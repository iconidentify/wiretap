...
/**
 * Minimal TCP proxy that forwards AOL TCP traffic and emits JSONL summaries live to the LiveBus (SSE).
 * Note: This is a simple implementation, not optimized for high throughput.
 */
public final class RealtimeAolSniffer {
        ...
        public void run() {
            try {
                ...
                while (true) {
                    ...
                    outStr.flush();
                    // naive scan for AOL frames in this chunk
                    List<FrameSummary> frames = new ArrayList<>();
                    scanAol(buf, 0, n, c2s ? "C->S" : "S->C", frames);
    ...
    // Lightweight scanner mirroring AolExtractor.scanFrames/summarize behavior for streaming chunks
    private static void scanAol(byte[] a, int start, int end, String dir, List<FrameSummary> out) {
        int i = start;
        while (i + 6 <= end) {
