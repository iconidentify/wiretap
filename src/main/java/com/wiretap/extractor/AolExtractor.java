...
public final class AolExtractor {
    ...
    public void extract(String pcapPath,
        ...
        java.util.logging.Logger.getLogger("pcap-extractor").info("[AOL-PCAP] reading: " + pcapPath);
        ...
        try (PcapReader reader = new PcapReader(pcapPath)) {
            ...
            java.util.logging.Logger.getLogger("pcap-extractor").info("[AOL-PCAP] processed " + packetCount + " packets");
        }

        java.util.logging.Logger.getLogger("pcap-extractor").info("[AOL-PCAP] frames parsed: " + frames.size());

        if (frames.size() == 0) {
            java.util.logging.Logger.getLogger("pcap-extractor").warning("[AOL-PCAP] DIAGNOSTIC: No frames parsed!");
            ...
            if (!sawTcpPort) {
                java.util.logging.Logger.getLogger("pcap-extractor").info("  - HINT: No TCP port " + serverPort + " traffic found - check if this is AOL traffic");
            }
        }
        ...
        if (storeFull && !repeatCounts.isEmpty()) {
            java.util.logging.Logger.getLogger("pcap-extractor").info("[AOL-PCAP] dedup stats: " + repeatCounts.size() + " repeated frames");
        }
    }
    ...
    private static int scanFrames(byte[] a, int start, int end, String dir, Instant ts, List<FrameSummary> out) {
        int i = start;
        while (i + 6 <= end) {
            if ((a[i] & 0xFF) != AOL_MAGIC) { i++; continue; }
